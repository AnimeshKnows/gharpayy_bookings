...
export const getGetBookingReminderUrl = (id: number) => `/api/bookings/${id}/reminder`;

export const getBookingReminder = async (
  id: number,
  options?: RequestInit,
): Promise<WhatsappMessage> => {
  return customFetch<WhatsappMessage>(getGetBookingReminderUrl(id), {
    ...options,
    method: "GET",
  });
};

export const getGetBookingReminderQueryKey = (id: number) => {
  return [`/api/bookings/${id}/reminder`] as const;
};

export const getGetBookingReminderQueryOptions = <
  TData = Awaited<ReturnType<typeof getBookingReminder>>,
  TError = ErrorType<unknown>,
>(
  id: number,
  options?: {
    query?: UseQueryOptions<Awaited<ReturnType<typeof getBookingReminder>>, TError, TData>;
    request?: SecondParameter<typeof customFetch>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};
  const queryKey = queryOptions?.queryKey ?? getGetBookingReminderQueryKey(id);
  const queryFn: QueryFunction<Awaited<ReturnType<typeof getBookingReminder>>> = ({ signal }) =>
    getBookingReminder(id, { signal, ...requestOptions });
  return { queryKey, queryFn, enabled: !!id, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getBookingReminder>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export function useGetBookingReminder<
  TData = Awaited<ReturnType<typeof getBookingReminder>>,
  TError = ErrorType<unknown>,
>(
  id: number,
  ...
  const queryOptions = getGetBookingReminderQueryOptions(id, options);
...
import type { CreateBookingRequestBody, Booking } from "./api.schemas";
...
export const createBookingRequest = async (
  ...
  return customFetch<Booking>(getCreateBookingRequestUrl(), {
    ...options,
    method: "POST",
...
export type CreateBookingRequestMutationBody = CreateBookingRequestBody;
export type CreateBookingRequestMutationError = ErrorType<unknown>;

/**
 * @summary Submit a self-service room booking request (tenant flow)
 */
...
> {
  ...
  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof createBookingRequest>>,
    { data: CreateBookingRequestBody }
  > = ({ data }) => createBookingRequest(data, requestOptions);
